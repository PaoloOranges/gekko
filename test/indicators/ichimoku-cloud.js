...
const TREND = require(dirs.indicators + 'ICHIMOKU-CLOUD-TYPES').TREND;
const CLOUD_COLOR = require(dirs.indicators + 'ICHIMOKU-CLOUD-TYPES').CLOUD_COLOR;
...
describe('indicators/ICHIMOKU-CLOUD', function () {
  ...
  const verifiedCloudColor = [CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.RED, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN, CLOUD_COLOR.GREEN];
  // 52 days shift as spanB
  const verifiedTrend = [TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.FLAT, TREND.UP, TREND.FLAT, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.FLAT, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.UP, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.DOWN, TREND.FLAT, TREND.FLAT, TREND.FLAT, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP, TREND.UP];
  ...
  it('should correctly calculate SenkouSpan A with default weight 9-26 and displacement 26', function () {
    let configPeriodsDisplacement = Object.assign({}, configPeriods);
    configPeriodsDisplacement.displacement = 26
    let ichimokuCloud = new ICHIMOKUCLOUD(configPeriodsDisplacement);
    _.each(prices, function (p, i) {
      ichimokuCloud.update(p);
      if (i >= configPeriodsDisplacement.kijunsen + configPeriodsDisplacement.displacement - 1) {
        var j = i - (configPeriodsDisplacement.kijunsen + configPeriodsDisplacement.displacement) + 1;
        expect(ichimokuCloud.result.senkouSpanA).to.closeTo(verifiedSenkouSpanA[j], 0.0001);
      }
    });
  });
  ...
  it('should correctly calculate SenkouSpan B with default weight 52', function () {
    ...
    _.each(prices, function (p, i) {
      ...
      if (i >= configPeriods.senkouspanb - 1) {
        let j = i - (configPeriods.senkouspanb - 1);
        expect(ichimokuCloud.result.senkouSpanB).to.closeTo(verifiedSenkouSpanB[j], 0.0001);
      }
    });
  });

  it('should correctly calculate SenkouSpan B with default weight 52 with displacement 26', function () {
    let configPeriodsDisplacement = Object.assign({}, configPeriods);
    configPeriodsDisplacement.displacement = 26
    let ichimokuCloud = new ICHIMOKUCLOUD(configPeriodsDisplacement);
    _.each(prices, function (p, i) {
      ichimokuCloud.update(p);
      if (i >= configPeriods.senkouspanb + configPeriodsDisplacement.displacement - 1) {
        let j = i - (configPeriodsDisplacement.senkouspanb + configPeriodsDisplacement.displacement) + 1;
        expect(ichimokuCloud.result.senkouSpanB).to.closeTo(verifiedSenkouSpanB[j], 0.0001);
      }
    });
  });

  it('should correctly give cloud color after senkouSpan B periods', function () {
    let ichimokuCloud = new ICHIMOKUCLOUD(configPeriods);
    _.each(prices, function (p, i) {
      ichimokuCloud.update(p);
      if (i >= configPeriods.senkouspanb - 1) {
  ...
  it('should correctly give cloud color after senkouSpan B periods and displacement 26', function () {
    let configPeriodsDisplacement = Object.assign({}, configPeriods);
    configPeriodsDisplacement.displacement = 26
    let ichimokuCloud = new ICHIMOKUCLOUD(configPeriodsDisplacement);
    _.each(prices, function (p, i) {
